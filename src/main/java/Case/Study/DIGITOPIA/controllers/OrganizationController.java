...
public class OrganizationController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteOrganization(@PathVariable("id") UUID organizationId) {
        organizationService.deleteOrganization(organizationId);
        return ResponseEntity.ok().build();
    }

    @GetMapping("/search-organization")
    public ResponseEntity<Page<OrganizationResponse>> searchOrganizationByRegistryNumber(@RequestParam String registryNumber,  @ParameterObject Pageable pageable){
